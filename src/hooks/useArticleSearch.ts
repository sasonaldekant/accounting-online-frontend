...
/**
 * 🔍 Server-side search hook za artikle (11000+ records)
 * 
 ...
 * @returns React Query result sa ArticleComboDto[]
 * 
 * @example
 * ```tsx
 * const { data: articles, isLoading } = useArticleSearch('crna', 50);
 * ```
 */
export const useArticleSearch = (
  ...
  return useQuery(
    ['articles', 'search', searchTerm, limit] as const,
    async () => {
      // Ne zovemo API ako je search term prekratak
      if (!searchTerm || searchTerm.trim().length < 2) {
        return [];
      }
      return api.lookup.searchArticles(searchTerm.trim(), limit);
    },
    {
